import {
  ...
  strings,} from "./codigos.js";
import { MARCAS } from "./codigos.js";

document.querySelector("#ayuda").addEventListener("click", () => {
  window.open(
    "https://ayuda.tigo.com.co/hc/es/articles/360000125228--C%C3%B3mo-configurar-mi-control-Universal-Tigo-Hogar"
  );
});



let query = document.querySelector("#tipo");
query.addEventListener("click", (e) => {
  const padre = document.getElementById("marcas").value;
  const hijo = e.target;
  const tipo = hijo.closest(".btns").value;
  ...
  let found = MARCAS.find((item) => item === codigos);
  if (!found) {
    toString(
      swal.fire({
        width: "300px",
    ...
    //debe ser al final el else o sino  return;false me comenta todo el resto del codigo//
      ...
      toString(
        Swal.fire({
          width: "300px",
          icon: "error",
          title: "realize la busqueda de nuevo codigos no encontrados",
          timer: 60000,
          allowOutsideClick: false,
          color: "black",
        })
...
const pg1 = document.getElementById("televisor").addEventListener("click", () => {
   window.location.href = "televisor.html";
 });
 
const pg2 = document.getElementById("decodificador").addEventListener("click", () => {
  window.location.href = "decodificador.html";
});
